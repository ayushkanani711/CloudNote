...
//             <input type="password" className="form-control" id="cpassword"  onChange={onChange} placeholder='Confirm password' name="cpassword"  minLength={5} required />
//         </div>
...
const Signup = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await fetch(
      `https://i-notebook-backend-omega.vercel.app/api/auth/createuser`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
    ...
    if (json.success) {
      ...
      showalert('Please try to login with a new email', 'warning');
    }
  };
